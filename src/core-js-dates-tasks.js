...
function getDayName(date) {
  ...
  return daysName[dateObj.getDay()];
}
...
function getNextFridayThe13th(date) {
  ...
  let isFridayThe13thFind = false;

  let currentDate;

  while (!isFridayThe13thFind) {
    currentDate = new Date(currentYear, currentMonth, 13);
    const currentDayIndex = currentDate.getDay();

    if (currentDayIndex === 5) {
      isFridayThe13thFind = true;
    }
    currentMonth += 1;
  }
...
function getQuarter(date) {
  ...
  if (countMonth >= 4 && countMonth <= 6) {
    return 2;
  }
  if (countMonth >= 7 && countMonth <= 9) {
    return 3;
  }
  return 4;
}
